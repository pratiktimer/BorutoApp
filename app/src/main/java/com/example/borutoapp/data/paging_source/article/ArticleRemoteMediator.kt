...
class ArticleRemoteMediator(
    ...
    private val articleRemoteKeysDao = borutoDatabase.articleRemoteKeysDao()

    override suspend fun initialize(): InitializeAction {
        val currentTime = System.currentTimeMillis()
        val lastUpdated = articleRemoteKeysDao.getRemoteKeys(heroId = 1)?.lastUpdated ?: 0L
    ...
    override suspend fun load(loadType: LoadType, state: PagingState<Int, Article>): MediatorResult {
        return try {
            ...
            val response = borutoApi.getAllArticles(page = page)
            if (response.data.isNotEmpty()) {
                borutoDatabase.withTransaction {
                    ...
                    articleRemoteKeysDao.addAllRemoteKeys(heroRemoteKeys = keys)
                    articleDao.addArticles(heroes = response.data)
                }
            }
    ...
    private suspend fun getRemoteKeyClosestToCurrentPosition(
        ...
        return state.anchorPosition?.let { position ->
            state.closestItemToPosition(position)?.id?.let { id ->
                articleRemoteKeysDao.getRemoteKeys(heroId = id)
            }
        }
    }
    ...
    private suspend fun getRemoteKeyForFirstItem(
        ...
        return state.pages.firstOrNull { it.data.isNotEmpty() }?.data?.firstOrNull()
            ?.let { hero ->
                articleRemoteKeysDao.getRemoteKeys(heroId = hero.id)
            }
    }
