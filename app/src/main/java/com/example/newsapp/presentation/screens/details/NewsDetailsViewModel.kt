...
class NewsDetailsViewModel @Inject constructor(
    ...
    private val _selectedHero: MutableStateFlow<Article?> = MutableStateFlow(null)
    val selectedHero: StateFlow<Article?> = _selectedHero

    init {
        viewModelScope.launch(Dispatchers.IO) {
            val heroId = savedStateHandle.get<Int>(DETAILS_ARGUMENT_KEY)
            _selectedHero.value = heroId?.let { useCases.getSelectedArticleUseCase(heroId = heroId) }
        }
    }
